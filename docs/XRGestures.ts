...
class XRGestures extends THREE.EventDispatcher{ 
    ...
    controller1 : THREE.Group = new THREE.Group();
    controller2 : THREE.Group = new THREE.Group();
        ...
        if (this.type === 'unknown' && this.touch){
            if (data1.startPosition !== undefined){
                if (this.multiTouch){
                    ...
                    const dist = data1.startPosition.distanceTo( this.controller1.position );
                    ...
                    if ( dist > 0.01 && velocity > 0.1 ){
                        ...
                        const maxY = (Math.abs(v.y) > Math.abs(v.x)) && (Math.abs(v.y) > Math.abs(v.z));
                        if ( maxY )
                        this.type = "swipe";
