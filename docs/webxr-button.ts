...
let _WEBXR_UI_CSS_INJECTED :any = {};
 ...
 * @private
 */
const generateInnerHTML = (cssPrefix : any, height : number)=> {
 ...
 */
const createDefaultView = (options : any)=> {
 ...
 * @return {string}
 */
const generateCSS = (options :any, fontSize=18)=> {
...
export class WebXRButton {
  ...
  domElement : any;
  __defaultDisplayStyle = 'initial';
  __forceDisabled = false;
  ...
  setTitle(text : any) {
    ...
    ifChild(this.domElement, this.options.cssprefix, 'title', (title :any)=> {
      if (!text) {
   ...
   * @private
   */
  __setDisabledAttribute(disabled : boolean) {
 ...
 * @private
 */
const ifChild = (el :any, cssPrefix : string, suffix : string, fn :any)=> {
